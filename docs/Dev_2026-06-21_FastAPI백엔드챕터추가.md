...
- 업그레이드(화려하게) 시도 → 산만하다는 피드백으로 **정리**: 글로우·다색·반짝임·마우스 헤일로·클릭 리플·오로라 블롭 모두 제거. **단색(블루) 점이 천천히 움직이며 가는 선으로 연결되는 깔끔한 라인 네트워크**로 회귀(선의 움직임 수준). 점 수 상한 56·옅은 opacity(선 0.14·점 0.4)로 정돈. 마우스 가벼운 끌림+연결선만 유지. 라이브 확인 완료.
